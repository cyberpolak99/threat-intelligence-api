...
def init_db():
    ...
    cursor.execute('''INSERT OR IGNORE INTO incident_reports
        (id, ip_address, threat_type, severity, source, detected_at, description) VALUES
        (1,'192.168.1.100','malware','high','CERT-PL',datetime('now'),'Suspicious activity'),
        (2,'10.0.0.55','phishing','medium','VirusTotal',datetime('now'),'Phishing attempt'),
        (3,'172.16.0.1','brute_force','low','Honeypot',datetime('now'),'SSH brute force')''')
    conn.commit()
    conn.close()
